...
class CharacterListFragment : Fragment() {
        ...
        recyclerView.addOnScrollListener(object :RecyclerView.OnScrollListener(){
                override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                    ...
                    if (layoutManager.findLastVisibleItemPosition()== layoutManager.itemCount-1){
                        paginatedValue += 20
                        characterListViewModel.getCharacters(paginatedValue)
                        observeList()
                    }
                }
            })
    ...
    private fun recyclerViewProperties(){
        characterListAdapter = CharacterListAdapter(this.requireContext(), mutableListOf(),findNavController())
    ...
    private fun observeList(){
        lifecycle.coroutineScope.launchWhenCreated {
                characterListViewModel.marvelList.observe(viewLifecycleOwner) { it ->
                    ...
                    if (it.characterList.isNotEmpty()) {
                        binding.progressBar.visibility = View.GONE
                        flag = 0
